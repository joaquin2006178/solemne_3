...
# Página de Interacción con Datos
    ...
    valor_minimo = st.slider(f"Valor mínimo para {columna_filtrar}", min_value=int(df_paises[columna_filtrar].min()), max_value=int(df_paises[columna_filtrar].max()))
    df_filtrado = df_paises[df_paises[columna_filtrar] >= valor_minimo]
    st.subheader(f"Datos Filtrados (Columna: {columna_filtrar} >= {valor_minimo})")
...
# Página de Visualización Gráfica
    ...
    # Slider para el rango de X
    x_slide = st.slider(
        f"Rango del eje X ({x_var})",
        min_value=float(min_x),
        max_value=float(max_x),
        value=(float(min_x), float(max_x))
        )
    # Slider para el rango de Y
    y_slide = st.slider(
        f"Rango del eje Y ({y_var})",
        min_value=float(min_y),
        max_value=float(max_y),
        value=(float(min_y), float(max_y))
        )
    # Filtrar los datos según los rangos seleccionados
    df_filtrado = df_paises[
        (df_paises[x_var] >= x_slide[0]) & (df_paises[x_var] <= x_slide[1]) &
         (df_paises[y_var] >= y_slide[0]) & (df_paises[y_var] <= y_slide[1])
         ]
    # Gráfico: Barra, Dispersión, Histograma
    fig, ax = plt.subplots()
